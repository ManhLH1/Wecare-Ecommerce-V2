...
export default function IndustrialVacuumShowcaseV2() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-2 py-4 bg-white my-4">
      {/* Top header tab */}
      ...
      <div className="bg-blue-500 rounded-b-xl rounded-tr-xl px-4 py-3 relative overflow-hidden">
              ...
              <div className="flex gap-6 items-stretch px-4 flex-nowrap">
                {loading ? (
                  Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="iv2-card bg-white rounded-lg shadow p-4 flex-shrink-0" style={{ width: 300 }}>
                  ...
                  products.map((p: any, i: number) => (
                    <div key={p.id || i} className="iv2-card bg-white rounded-lg shadow p-0 flex-shrink-0 snap-start" style={{ width: 300 }}>
                      <div className="relative h-44">
                        {p.image ? <Image src={p.image} alt={p.name} fill className="object-contain p-4" /> : <div className="w-full h-full bg-gray-100" />}
                        {/* discount badge removed per request */}
                      </div>
                      <div className="p-4 flex flex-col justify-between h-[200px]">
                        <div>
                          <div className="text-sm font-semibold text-gray-800 mb-3 line-clamp-3">{p.name}</div>
                          <div className="flex items-center">
                            <div className="text-xs text-gray-400 line-through">{p.price ? (p.price).toLocaleString("vi-VN") + " ₫" : ""}</div>
                            <div className="ml-auto">
                              <div className="inline-block px-4 py-3 bg-gradient-to-b from-yellow-300 to-yellow-400 text-red-700 font-extrabold rounded-lg text-lg">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          ...
                          <div className="text-xs bg-amber-100 text-amber-700 px-3 py-1 rounded-full">Đã bán {p.sold || ""}</div>
                        </div>
                      </div>
                    </div>
                  ))
