...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Validate GUID format to avoid invalid requests to Dynamics
    ...
    // Deactivate record by setting statecode = 1 (Inactive)
    const endpoint = `${BASE_URL}${SALE_ORDER_DETAILS_TABLE}(${detailId})`;
    try {
      await axios.patch(endpoint, { statecode: 1 }, { headers });
    } catch (err: any) {
      console.error("Dynamics error during deactivate:", err.response?.data || err.message);
    ...
    console.error("Error deactivating sale order detail:", error);
    if (error.response) {
