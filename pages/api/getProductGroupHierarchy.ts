...
const buildHierarchy = (productGroups: ProductGroup[]): ProductGroup[] => {
  ...
  // Calculate total product count for each node
  const calculateProductCount = (node: ProductGroup): number => {
    let totalCount = node.productCount || 0;
    
    if (node.children && node.children.length > 0) {
      node.children.forEach(child => {
        totalCount += calculateProductCount(child);
      });
    }
  ...
  // Calculate product counts for all root nodes
  rootGroups.forEach(calculateProductCount);
...
// ======= Main Handler =======
const getProductGroupHierarchy = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const PRODUCT_GROUP_COLUMNS =
      ...
      "crdfd_nhomsanphamchatext,_crdfd_nhomsanphamcap1_value,crdfd_image_url,cr1bb_so_san_pham_co_gia";
    ...
    console.error("Error in getProductGroupHierarchy:", error);
...
export default getProductGroupHierarchy; 